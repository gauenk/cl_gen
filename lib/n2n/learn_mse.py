...
from pyutils import sample_subset_grids

def compute_bootstrap_loss(samples,B,T,R=100):
    device = samples.device
    samples = rearrange(samples,'(t b) c h w -> t (c h w) b',t=T)
    ...
    scores_t = torch.zeros(T,B,device=device)
    counts_t = torch.zeros(T,1,device=device)
    batch_size = R//10
    nbatches = R // batch_size # break up computation into batches
...
def train_loop_mse(cfg,model,optimizer,criterion,train_loader,epoch):
    ...
    for batch_idx, (burst, res_img, raw_img, d) in enumerate(train_loader):
        ...
        loss_other = (1/(cfg.global_step+1.))**1.2 * F.mse_loss(burst,denoised)
        loss += loss_other
    ...
    with torch.no_grad():
        for batch_idx, (burst, res_img, raw_img, d) in enumerate(test_loader):
            ...
            if (batch_idx % cfg.test_log_interval) == 0:
                root = Path(f"{settings.ROOT_PATH}/output/mse/rec_imgs/e{epoch}")
