...
def train_loop_offset(cfg,model,optimizer,criterion,train_loader,epoch):
    ...
    for batch_idx, (burst_imgs, res_imgs, raw_img) in enumerate(train_loader):
        ...
        # -- direct denoising --
        # rec_img_i,rec_img,temporal_loss = model(cat_burst,stacked_burst)
        ...
        loss = np.sum(loss)# + temporal_loss
        kpn_loss = loss
        # temporal_loss = temporal_loss.item()
        # mse_loss = F.mse_loss(rec_img,mid_img)

        # -- compute ot loss to optimize --
        residuals = rec_img_i - t_img.unsqueeze(1).repeat(1,N,1,1,1)
        ...
        if (batch_idx % cfg.log_interval) == 0 and batch_idx > 0:
            ...
            write_info = (epoch, cfg.epochs, batch_idx,len(train_loader),running_loss,
                          psnr_ave,psnr_std,bm3d_nb_ave,bm3d_nb_std,ave_temporal_loss)
            print("[%d/%d][%d/%d]: %2.3e [PSNR]: %2.2f +/- %2.2f [bm3d]: %2.2f +/- %2.2f [loss-t]: %.2e" % write_info)
