...
from layers.ame_kpn.KPN_1f import KPN_1f,KPN_1f_fs64
...
def load_burst_n2n_model(cfg):
    ...
    disc_optim = load_optimizer(cfg,disc_model)
...
def load_burst_kpn_model(cfg):
    ...
    # -- create burstaligned model --
    model = BurstAlignSG(kpn,denoiser_info,noise_critic)
    criterion = BurstRecLoss(noise_critic,alpha=1.0)
...
def load_model_kpn_1f(cfg):
    if cfg.dynamic.frame_size == 128:
        return KPN_1f(color=True,kernel_size=[9],burst_length=cfg.input_N,blind_est=True),LossFunc(tensor_grad=~cfg.blind,alpha=1.0)
    else:
        return KPN_1f_fs64(color=True,kernel_size=[9],burst_length=cfg.input_N,blind_est=True),LossFunc(tensor_grad=~cfg.blind,alpha=1.0)
