...
class TransformerNetwork32_dip_v2(nn.Module):
    ...
    def forward(self, inputs, outputs, step, max_steps, loss_diff):
        ...
        # in_patches = self.end_conv(in_patches)
        ...
        for patch in range(n_patches):
            ...
            if self.use_pos_enc: ifp_enc = self.pos_enc(in_patches_flat[patch])
            else: ifp_enc = in_patches_flat[patch]
            for i in range(N):
                if self.use_pos_enc: ip_enc = self.pos_enc(in_patches[patch][i])
                else: ip_enc = in_patches[patch][i]
                # a = self.stn(in_patches[patch][i].reshape(1,3,32,32))
                # a = a[0].reshape(32*32,1,3)
                if self.use_pos_enc_values:
                    a,b = self.attn[i](ip_enc,ifp_enc,ifp_enc)
                else:
                    a,b = self.attn[i](ip_enc,ifp_enc,in_patches_flat[patch])
            ...
            # align_imgs = self.unet(align_imgs.contiguous())
            # print(nin_patch_mid.shape,align_imgs.shape,nin_patch.shape)
            
            std_est = 25./255
            align_imgs_cmp = align_imgs + torch.normal(0,std_est * torch.ones_like(align_imgs))
            mid_mse_loss = F.mse_loss(nin_patch_mid,align_imgs_cmp)
            loss += mid_mse_loss
            mid_mse_loss_item = mid_mse_loss.item()

            # -- [todo] swap noise patterns; (i) additive noise + (ii) subtracting diff function implies we just compare directly --

            # -- compare _difference_ between frames --            
            # loss += F.mse_loss( torch.pow(align_imgs[1] - align_imgs[0],2), torch.pow(align_imgs[2] - align_imgs[1],2) )
            # loss += torch.abs(torch.mean(torch.pow(align_imgs[1] - align_imgs[0],2)) - torch.mean( torch.pow(align_imgs[2] - align_imgs[1],2) )) / 3.
            # loss += torch.abs(torch.mean(torch.pow(align_imgs[1] - align_imgs[0],2)) - torch.mean( torch.pow(align_imgs[2] - align_imgs[0],2) )) / 3.
            # loss += torch.abs(torch.mean(torch.pow(align_imgs[2] - align_imgs[1],2)) - torch.mean( torch.pow(align_imgs[2] - align_imgs[0],2) )) / 3.

            # -- TODO: replace the mse with wasserstein loss --
            if self.use_all_loss:
                all_mse_loss = F.mse_loss(nin_patch,align_imgs)
                all_mse_loss_item = all_mse_loss.item()
                # print("m",mid_mse_loss / all_mse_loss_item)
                if self.all_loss_v == "v1":
                    loss += all_mse_loss 
                elif self.all_loss_v == "v2":
                    loss += (1 - step/max_steps)**2 * all_mse_loss 
                elif self.all_loss_v == "v3":
                    loss += loss_diff * (1 - step/max_steps)**2 * all_mse_loss 
                else:
                    raise ValueError(f"Unknown all loss verion [{self.all_loss_v}]")
            ...
            mean_img_n = torch.mean(align_imgs,dim=0).contiguous()
            # diff_pre = nin_patch_mid - mean_img_n 

            # align_res = self.unet(align_imgs)
            # mean_img = torch.mean(align_imgs - align_res,dim=0).contiguous()
            # diff_post = nin_patch_mid - mean_img
            # mean_img = torch.mean(self.unet(align_imgs),dim=0).contiguous()

            # loss += F.mse_loss(diff_pre,diff_post)
            # mean_img = torch.mean(align_imgs,dim=0).contiguous()
            # conv_in = rearrange(align_imgs,'n c h w -> 1 (n c) h w')
            # rec_patch = mean_img
            # conv_in = mean_img.unsqueeze(0)
            # rec_patch = self.end_conv(conv_in)[0]
            # res_patch = self.unet(conv_in)[0]
            # rec_patch = nin_patch_mid - res_patch
            rec_patch = mean_img_n
            # loss += 10*(1 - step/max_steps)**2 * F.mse_loss(nin_patch[N//2],mean_img_n)

            # print('r2',rec_patch.shape)
            # loss += F.mse_loss(mean_img,rec_patch)
        ...
        std_est = (25./255) / np.sqrt(3)  # torch.mean( torch.pow(outputs[:,N//2] - rec_image,2) )
        # print("std_est",255*std_est.item(),255*torch.std(outputs[:,N//2] - torch.mean(outputs[:,N//2])).item())
        noisy_rec = rec_image + torch.normal(0,torch.ones_like(rec_image)*std_est)
        rec_loss = F.mse_loss(outputs[:,N//2],noisy_rec)
...
class WLoss():
    ...
    def __init__(self,dim,gpuid,iters=5,init_lr=0.1):
        self.gpuid = gpuid
        self.iters = iters
    ...
    def __call__(data,iters=None):
        self.model.train()
        if iters is None: iters = self.iters
        data = data.cpu().cuda(self.gpuid)
        for i in range(iters):
            loss = self.model(data)
        raise NotImplemented("Need many batches of images.")
