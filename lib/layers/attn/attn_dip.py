...
class TransformerNetwork32_dip(nn.Module):
    ...
    def __init__(self, ftr_model, d_model, im_size, input_patch, output_patch, n_frames, bw=False, denoise_model = None, batch_size = 8):
        ...
        self.perform = Performer(d_model,3,8) # 8,4
    ...
    def forward(self, inputs, outputs):
        ...
        for patch in range(n_patches):
            ...
            # -- plot attn map --
            plt.imshow(w_attn.detach().cpu())
            plt.savefig("w_attn.png",dpi=500)
            plt.close("all")
            ...
            # print(nin_patch_mid.shape,align_imgs.shape)
            # loss += F.mse_loss(nin_patch_mid,align_imgs)
            # print(loss.item())
            ...
            inp = rearrange(in_patches[patch],'(n i_h i_w) 1 c -> n c i_h i_w',i_h=i_p,i_w=i_p).detach().cpu()
        ...
        loss += F.mse_loss(outputs[:,N//2],rec_image)
