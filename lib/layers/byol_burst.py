...
class BurstAlignSG(nn.Module):
    ...
    def __init__(self, kpn, denoiser_info, noise_critic, skip_middle = True):
...
class BurstRecLoss(nn.Module):
    ...
    """
    def __init__(self, noise_critic, coeff_ave=1.0, coeff_burst=100.0, coeff_ot=100.0, gradient_L1=True,
    ...
    def forward(self, aligned, aligned_ave, rec_img, ground_truth, sm_raw_img, global_step):
        ...
        loss_burst = self.coeff_burst * self.loss_burst(aligned, ground_truth)
        ...
        loss_ot = torch.FloatTensor([-1.]).to(aligned.device)

        # -- noise pattern loss using critic --
        # residual = aligned - sm_raw_img.unsqueeze(1).repeat(1,N,1,1,1)
        residual = aligned - ground_truth.unsqueeze(1).repeat(1,N,1,1,1)
        # loss_nc = 0 * self.noise_critic.compute_fake_loss(residual)
        loss_nc = torch.FloatTensor([-1.]).to(aligned.device)
