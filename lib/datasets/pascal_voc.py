...
from transforms import get_dynamic_transform,get_noise_transform
...
class DenoiseVOC(VOCDetection):
    ...
    def __getitem__(self, index):
        ...
        if self._return_type == "list":
            ...
            raise ValueError("How did you change this?")
...
class DynamicVOC(VOCDetection):
    ...
    def __getitem__(self, index):
        ...
        if self._return_type == "list":
            ...
            raise ValueError("How did this happen? Invalid return type [{self.return_type}]")

class DynamicVOC_LMDB():

    def __init__(self,lmdb_path,metadata_fn,N,noise_type,noise_params,dynamic_info):
        ...
        self.dynamic_info = dynamic_info
        self.size = self.dynamic_info.frame_size
        self.fields = ['burst','sim_burst','raw','direction']
    ...
    def _read_img_noise_lmdb(self, data_env, lmdb_index, dtype=np.float32, shape=(3,128,128)):
        ...
        # -- create lmdb keys --
        keys = {}
        for field in self.fields: keys[field] = "{}_{}".format(lmdb_index,field).encode('ascii')
...
def get_voc_dataset(cfg,mode):
    ...
    if mode == 'cl':
        ...
        data.val = DynamicVOC(root,"2012","val",N,noise_type,noise_params,dynamic,load_res,bw,rtype)
        # data.val.data = data.val.data[0:2*2048]
        # data.val.targets = data.val.targets[0:2*2048]
        data.te = DynamicVOC(root,"2007","test",N,noise_type,noise_params,dynamic,load_res,bw,rtype)
    elif mode == "dynamic-lmdb":
        batch_size = cfg.batch_size
        N = cfg.N
        load_res = cfg.dataset.load_residual
        noise_type = cfg.noise_type
        ...
        data.tr = DynamicVOC_LMDB(lmdb_path,metadata_fn,N,noise_type,noise_params,dynamic)
