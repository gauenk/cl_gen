...
"""
The motivation to synthetically create local motion
is to assess specific components of motion parameters.

The local motion parameters should include:
- number of locally moving objects

for each local object we want to know:
- absolute size of local objects
- relative size of local objects
- smooth/texture of local object

the motion of each local object:
- change of radiation condition
- change of scale
- rotation

We want two fields for local motion:
(i) a flow fields (2,H,W)
(ii) a corespondence field (N2,H,W) 
    for neighborhood of size N, N2 = N*N
"""
...
class LocalCameraMotionTransform():
    """
    Axis-aligned motion
