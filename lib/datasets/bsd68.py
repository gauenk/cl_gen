...
class DenoiseBSD68(BSD68Detection):
...
class DynamicBSD68(BSD68Detection):
...
def get_bsd68_dataset(cfg,mode):
    ...
    if mode == 'cl':
        ...
        dynamic = cfg.dynamic
        bw = cfg.dataset.bw
        data.tr = DynamicBSD68(root,"2012","trainval",N,noise_type,noise_params,dynamic,load_res,bw)
        data.val = DynamicBSD68(root,"2012","val",N,noise_type,noise_params,dynamic,load_res,bw)
        # data.val.data = data.val.data[0:2*2048]
        # data.val.targets = data.val.targets[0:2*2048]
