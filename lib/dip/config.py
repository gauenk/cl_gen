"""
Configurations for DeepImagePrior experiments
...
def get_args():
    parser = argparse.ArgumentParser(description="Run a N2N Experiment")
...
def set_cfg(args):
    ...
    model_path = Path(f"{settings.ROOT_PATH}/output/dip/{dsname}/{cfg.exp_name}/model/")
    optim_path = Path(f"{settings.ROOT_PATH}/output/dip/{dsname}/{cfg.exp_name}/optim/")
