...
from layers.burst import BurstAlign,AlignLoss,NoiseCriticModel
from layers.denoise_gan import DCGAN_D
from .optim_io import load_optimizer

def load_burst_model(cfg):

    # -- init --
    init_lr = cfg.init_lr

    # -- load kpn model --
    kpn,_ = load_model_kpn(cfg)
    ...
    model = BurstAlign(kpn,unet_info,noise_critic)
    criterion = AlignLoss(noise_critic)
