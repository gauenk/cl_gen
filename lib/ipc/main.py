...
# inspect_parameter_change(init_cfg,init_model)
def generate_dataset(init_cfg,init_model,overwrite=False):

    """
    TODO:

    write "train_model" wrapper: 
    arg0: cfg
    arg1: init model parameters
    arg2: the number of training epochs


    create noise_params list

    write "create_cfg" function to link with noise_params list

    write "save_fxn" for pytorch models

    
    """

    noise_params_v1 = get_noise_param_level_v1()
    noise_params_v2 = get_noise_param_level_v2()
    noise_params = [noise_params_v1,noise_params_v2]

    results = []
    save_path = Path("")
    num_epochs = {'short':5,'medium':25,'long':50}
    D = len(noise_params)
    for i in range(D):
        for j in range(D):
            
            # -- init experiment --
            cfg_i = create_cfg(init_cfg,noise_params[i])
            cfg_j = create_cfg(init_cfg,noise_params[j])
            cfgs = {f"{i}":cfg_i,f"{j}":cfg_j}
            models = dict.fromkeys([f"{i}",f"{i}2{j}_s",f"{i}2{j}_m",f"{i}2{j}_l"])

            # -- execute experiment if not completed --
            completed = check_experiment_completed(save_path,cfgs,models,i,j)
            if not completed and overwrite is False:
                models[f"{i}"] = train_model(cfg_i,None,num_epochs.medium)
                models[f"{i}2{j}_s"] = train_model(cfg_j,model_i,num_epochs.short)
                models[f"{i}2{j}_m"] = train_model(cfg_j,model_i,num_epochs.medium)
                models[f"{i}2{j}_l"] = train_model(cfg_j,model_i,num_epochs.large)
                
            # -- prepare for appending --
            append_results(results,save_path,cfgs,models,i,j)

    return results

def check_experiment_completed(save_path,cfgs,models,i,j):
    exist_list = []
    def checkpath(path,obj): return exist_list.append(path.exists())
    save_object_list(save_path,checkpath,"models",models)
    save_object_list(save_path,checkpath,"cfgs",cfgs)
    completed = all(exist_list)
    return completed

def save_object_list(save_path,save_fxn,obj_name,objects):
    obj_fn = {}
    for name,obj in objects.items():
        obj_path = save_path / obj_name / name
        save_fxn(obj_path,obj)
        obj_fn[name] = obj_path
    return obj_fn

def append_results(results,save_path,cfgs,models,i,j):
    
    # -- create list of noise levels --
    noises = {key:create_noise_string(cfg) for key,cfg in cfgs.items()}

    # -- create save functions --
    def save_model(path,obj):
        if not path.endswith(".th"): path += ".th"
        torch.save(path,obj)
    def save_cfg(path,obj):
        if not path.endswith(".pkl"): path += ".pkl"
        with open(path,'wb') as f:
            pickle.dump(f,obj)

    # -- save objects to file and save filename --
    models_fn = save_object_list(save_path,save_model,"models",models)
    cfgs_fn = save_object_list(save_path,save_cfg,"cfgs",cfgs)

    # -- concatenate + append all results --
    result = noises + models_fn + cfgs_fn 
